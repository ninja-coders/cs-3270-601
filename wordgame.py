...
# the Outer game flag the the 'will begin agian' loop
    ...
    while (beginAgain):
        ...
        # choise pulls a random word from the list of AllWords read from the file
    ...
    # Inner Game loop to play once until all guesses have been  made
        ...
        #print(li2)
        #flag = False if success == len(possible_words) else True
        if not len(li3):
...
# Cody's Code
    ...
    userGuess = input("Enter a guess: ")
    if userGuess in possible_words:
        ind = li1[len(userGuess)-lo].index(userGuess)
        li2[len(userGuess)-lo].insert(ind, userGuess)
    
    print(li2)
    
    
    
    print("these are the words for each count starting at lo and approaching hi: ")
    print(li1)
    print()
    print("these are all the permutations of the word sorted by length: ")
    print(li2)
'''
# if i can make the sets created  to be equal were in business
'''
# My code

    li3 = []
    li4 = []
    for length, group in groupby(possible_words, len):

        quanto = len(list(group))
        print("--> There are ", quanto , " items of length", length," in your list to guess: ", (["_ " * length ]) * quanto)
        print(list(group))
        li3.append( {word for word in list(group)})
        li4.append(("_ " * length) * quanto)
        print()


    print("this list will be my display list: ")
    print(li4)
    print()
    print("here is a list of lists by length: ")
    print(li3)

    userGuess = input("Enter a guess: ")
    if userGuess in possible_words:
        ind = li3[len(userGuess)-lo].index(userGuess)
        li4[len(userGuess)-lo].insert(ind, userGuess)
